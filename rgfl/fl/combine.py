...
def combine_file_level(args):
    ...
    embed_used_map = {x["instance_id"]: x["found_files"] for x in embed_used_locs}
    ...
    for pred in tqdm(model_used_locs, colour="MAGENTA"):
        ...
        # always include model locations
        for loc in model_loc[: args.top_n]:
            combined_loc_counter[loc] += 1

        # if present, include retrieval locations
        if instance_id in embed_used_map:
            retrieve_loc = embed_used_map[instance_id]
